...
- **Investigate Metabase result pipeline overhead (`hasheq`)**: NixOS VM column-scaling benchmarks (1→100 cols x 20K rows) show that ClickHouse and PostgreSQL converge to identical performance at 50+ columns — the bottleneck is Metabase's core result processing, not the ClickHouse driver. JMC profiling ([discourse thread](https://discourse.metabase.com/t/metabase-x10-times-slower-than-the-sql-query/215352)) identifies `clojure.lang.Util.hasheq(Object)` as the CPU hotspot. Production measurements show 10x overhead (21ms SQL → 370ms API) even for small result sets (16-46 rows). The ~100ms baseline for SELECT 1 (both engines) suggests significant middleware overhead. The ClickHouse driver adds a small fixed cost (~100-150ms) at low column counts but this is dwarfed by `hasheq` at scale.
